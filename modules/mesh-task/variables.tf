...
variable "create_discoverer_role" {
  description = "Whether mesh-task will create the discoverer IAM role. Defaults to true. This must be set to false when passing in an existing role using the `discoverer_role` variable."
  type        = bool
  default     = true
}
...
variable "discoverer_role" {
  description = "ECS discoverer role to include in the task definition. You must also set `create_discoverer_role=false` so that mesh-task knows not to create a role for you."
  type = object({
    id  = string
    arn = string
  })
